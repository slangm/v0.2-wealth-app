...
class AllocationDto {
  @IsIn(["protected", "growth"], {
    message: "target must be protected or growth",
  })
...
export class PortfolioController {
  ...
  setup(@CurrentUser() user: User, @Body() dto: SetupDto) {
    const targets = this.service.setTargets(
      user.id,
      dto.safePct,
      dto.growthPct
    );
    const job = this.service.createDeploymentJob(
      user.id,
      dto.safePct,
      dto.growthPct
    );
  ...
  confirmBridge(@CurrentUser() user: User, @Body() dto: ConfirmBridgeDto) {
    ...
    job.logs.unshift(
      `${new Date().toISOString()} BRIDGE_CONFIRMED: user reported funds on Base Sepolia Growth wallet`
    );
